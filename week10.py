...
arduino_port = 'COM4'  # Adjust the serial port for Arduino as needed
baud_rate = 9600
...
def main():
    ...
    device_choice = input("Enter 'arduino' to connect to Arduino or 'controller' to connect to Motion Controller: ").strip().lower()
    if device_choice == 'arduino':
        ser = connect_to_device(arduino_port)
    elif device_choice == 'controller':
    ...
    if ser:
        try:
            # Example of how to call the align_pcb function
            image_path = 'F:\\Carbon Fiber Tip Preparation Files\\microwire_misaligned\\WIN_20240229_12_32_29_Pro.jpg'
            x_CF = 100  # Example x coordinate from user input
            y_CF = 200  # Example y coordinate from user input
            target = 9

            

            # Continue with your motor control logic using x_ct and y_ct
            motor_control(ser, device_choice)
        finally:
            ser.close()
