...
def open_camera(device_path, width=320, height=240, fps=15):
    cap = cv2.VideoCapture(device_path)
    if not cap.isOpened():
        print(f"Unable to open camera at {device_path}")
    else:
...
def camera_feed():
    camera1_path = '/dev/video2'
    camera1 = open_camera(camera1_path)
    ...
    try:
        while True:
            ...
            if not ret1 or frame1 is None:
                print(f"Failed to capture image from camera 1 at {camera1_path}.")
                break
