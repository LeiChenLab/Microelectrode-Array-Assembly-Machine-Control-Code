...
arduino_port = 'COM5'  # Adjust the serial port for Arduino as needed
baud_rate = 9600
...
def camera_feed():
    camera_index = 0  # Use the correct camera index for Windows
...
def motor_control(ser):
    while True:
        ...
        if command == 'move':
            ...
            stop_motor(ser)
            user_choice = input("Do you want to move the Arduino or Controller again? (yes/no): ").strip().lower()
            if user_choice == 'no':
                break
...
def main():
    ...
    # Prompt user to choose between Arduino and Controller
